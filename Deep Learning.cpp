...
bool policy [200]; //if true we hit
...
void step (int k, bool action)              //one turn of the player who already knows the action he is taking
{
  ...
  if(action) //hit
  {
      ...
      else
      {
          ...
          if(actionvaluefunction[nextstate][1] < actionvaluefunction[nextstate][0])
              actionvaluefunction[k][0] += (1./(float)stateactionpaivisits[k][0])*((float)reward + discountvalue * actionvaluefunction[nextstate][0] - actionvaluefunction[k][0]);
          else
              actionvaluefunction[k][0] += (1./(float)stateactionpaivisits[k][0])*((float)reward + discountvalue * actionvaluefunction[nextstate][1] - actionvaluefunction[k][0]);
      }
